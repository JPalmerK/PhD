...
# X) Fit models to each deployment gorups, as before  #######################################
  ...
  for(ii in 1:10){
    ...
    data_sub=subset(OccTable_daily, GroupId==unique(OccTable$GroupId)[ii])
    ...
    gammformula=list()
    gammformula[[1]]=as.formula(BNDTotOffset~Year+s(JulienDay, k=3, by= ShoreDist)+ShoreDist)
    gammformula[[2]]=as.formula(BNDTotOffset~ShoreDist+s(JulienDay, k=3, by= Year)+Year)
    gammformula[[3]]=as.formula(BNDTotOffset~ShoreDist+Year+s(JulienDay, k=3))
    gammformula[[4]]=as.formula(BNDTotOffset~ShoreDist+Year+JulienDay)
    
    
    
    
    QIC_val=c(Inf,Inf,Inf)
    AIC_val=c(Inf,Inf,Inf)
    ...
    for(jj in 1:4){
      ...
      if (class(mod) == "try-error"){QIC_val[jj]=Inf} else {
        ...
        temp=withWarnings(gamm(gammformula[[jj]], 
                               data   = data_sub,
                               correlation=corAR1(form = ~1|UnitLoc),
                               family=binomial,
                               Select=TRUE))
        ...
        if(length(temp$Warnings)<2){AIC_val[jj]=AIC(temp$Value)}
    ...
    finalmodel=gamm(gammformula[[which.min(AIC_val)]], 
                      data   = data_sub,
                      correlation=corAR1(form = ~1|UnitLoc),
                      family=binomial,
                      Select=TRUE)
    ...
    ModelTable$DeltaQIC[ii]=sort(AIC_val)[2]-sort(AIC_val)[1]
    ...
    ModelTable$ModelFormula[ii]=Reduce(paste, deparse(formula(finalmodel)))
      ...
      if(ii==1){
        ...
        fit$fit=Preds$fit
        fit$lwr=Preds$fit-(1.96*Preds$se.fit)
        fit$upr=Preds$fit+(1.96*Preds$se.fit)
       ...
       # fit=rbind(fit, cbind(newdat, predictvcv(finalmodel)) )
        ...
        temp$fit=Preds$fit
        temp$lwr=Preds$fit-(1.96*Preds$se.fit)
        temp$upr=Preds$fit-(1.96*Preds$se.fit)
        
        
        fit=rbind(fit, temp) 
        
        
        #fit=rbind(fit, cbind(newdat, predict(finalmodel))) 
            }
    }  
    

  # Look at model performance at all 30 locations
  Modperformance=data.frame(GroupId=character(length = length(unique(OccTable_daily$UnitLoc))),
                            UnitLoc=character(length = length(unique(OccTable_daily$UnitLoc))),
                            AUC_unit=numeric(length = length(unique(OccTable_daily$UnitLoc))),
                            Pres_unit=numeric(length = length(unique(OccTable_daily$UnitLoc))),
                            Abs_unit=numeric(length = length(unique(OccTable_daily$UnitLoc))))
  
  levels(Modperformance$GroupId)= levels(OccTable_daily$GroupId)
  levels(Modperformance$UnitLoc)= levels(OccTable_daily$UnitLoc)
  ...
  for(ii in 1:10){
    ...
    for(jj in 1:n){
      ...
      if(sum(subsub$BBOcc>0)){
      mm=CalcAUC(modlist[[ii]], data_sub = subsub, BinaryResponse_var = 'BBOcc')
      ...
      Modperformance$UnitLoc[count]=subsub$UnitLoc[1]}else{
        Modperformance$AUC_unit[count]=NaN
        Modperformance$Pres_unit[count]=NaN
        Modperformance$Abs_unit[count]=NaN
      }
      count=count+1
    }
    rm(subsub, mm)
...
# X)  Create coefficents table ###
  ...
  mod=modlist[[1]]
  coeffvals=t(data.frame(coefficients(mod$gam)))
  
  for(ii in 2:10){
    mod=modlist[[ii]]
    NewVals=data.frame(coefficients(mod$gam))
...
# Xx) Plot models and fits for each location ####
  ...
  ggplot(data=OccTable_daily) +
    ...
    scale_colour_manual(values=cbbPalette) +
    geom_point(data=subset(OccTable_daily, ShoreDist=='05'),
