...
gadm <- readRDS("/home/kpalmer/PHD/PhD/Chapter 4/R Code/GBR_adm1.rds") 
...
# Read in meta data
meta2=read.csv('/home/kpalmer/PHD/PhD/Deployment Information/SlopeAndAspect.csv')
...
getNOAA.bathy(lon1 = min(meta2$Lon)-1, lon2 =  max(meta2$Lon)+.5,
...
dat=expand.grid(Lon=seq(min(meta2$Lon)-.1,
                        max(meta2$Lon)+.4, length.out = 250),
                Lat=seq(min(meta2$Lat-.45),
                        max(meta2$Lat+.45), length.out = 250))
...
rm(list=setdiff(ls(), c("dat", 'NorthSea', 'NorthSea_raster', 'river_locs',
...
OccTable= read.csv('/home/kpalmer/PHD/PhD/Chapter 4/R Code/OccupancyTable_ThreePdets.csv')
...
pcadata = cbind(meta2$DistToSalmonRun,  meta2$SlopeMap, meta2$DistToShore, meta2$Depth_m)
# scale PCA data
...
# Get the ranges and clean out the model
SlopeRange = range(meta2$Slope)
DistToShoreRange = range(meta2$DistToShore)
DistToFeatureRange = range(meta2$DistToSalmonRun)
DepthRange = range(meta2$Depth_m)
...
# Add river locs
river_locs_text=data.frame(Rivername=factor(c("Esk",
                                              "Dee", 
                                              "Spey",
                                              "Tay Firth", 
                                              "Tweed",
...
river_locs_text$lonDeg=as.numeric(substr(river_locs_text$Lon, 1,3)) -
  as.numeric(substr(river_locs_text$Lon, 4,5))/60 - 
  as.numeric(substr(river_locs_text$Lon, 6,7))/60/60

river_locs_text$LatDeg=as.numeric(substr(river_locs_text$Lat, 1,2)) +
  as.numeric(substr(river_locs_text$Lat, 3,4))/60 + 
  as.numeric(substr(river_locs_text$Lat, 5,6))/60/60

coordinates(river_locs_text)=c('lonDeg','LatDeg' )


proj4string(river_locs_text) <- crs.geo  # define projection system of our data
summary(river_locs_text)
...
# Set up fixed objects (north sea, river locs and river loc labels)
northsea_fort = fortify(NorthSea)
riverlocsdf = data.frame(river_locs)
riverlocstextdf = data.frame(river_locs_text)

# Contour levels and labels
contour_levels =  seq(min(northsea_fort),0, by=20)


# Set up east west labels
ewbrks <- round(seq(min(northsea_fort$x), max(northsea_fort$x),length.out = 4),1)
nsbrks <- round(seq(min(northsea_fort$y), max(northsea_fort$y), length.out = 4),1)
ewlbls <- unlist(lapply(as.character(ewbrks), function(x) ifelse(x < 0, paste(x, "°E"), ifelse(x > 0, paste(x, "°W"),x))))
nslbls <- unlist(lapply(as.character(nsbrks), function(x) ifelse(x < 0, paste(x, "°S"), ifelse(x > 0, paste(x, "°N"),x))))
...
theme_map <- function(...) {
  theme_minimal() +
    theme(
      ...
      #panel.grid.major = element_line(color = "#ebebe5", size = 1),
...
plot(NorthSea)
points(meta2, col='red', pch=18)
points(zz, col= 'pink', pch=18)

zz = data.frame(locator(10))
zz$group = c('Latheron', 'Helmsdale', 'Cromarty','Spey Bay', 'Fraserburgh', 'Cruden Bay',
             'Stonehaven', 'Arbroath', 'St. Andrews', 'St. Abbs')

# tweak text poxition of river locs
plot(NorthSea)
points(riverlocsdf$lonDeg, riverlocsdf$LatDeg, col='blue', pch=19)
points(river_titles_df, col='pink', pch=19)
river_titles_df =data.frame(locator(7))
river_titles_df$text= riverlocstextdf$Rivername


# Just location and CPOD names
require("rgdal") # requires sp, will use proj.4 if installed
require("maptools")
require("ggplot2")
require("plyr")
SAC = readOGR(dsn = '.', layer="SAC_SCOTLAND")
SAC=spTransform(SAC, crs.geo)
SAC@data$id = rownames(SAC@data)
SAC.points = fortify(SAC, region="id")
SAC.df = join(SAC.points, SAC@data, by="id")
MF_df = SAC.df[SAC.df$NAME == 'Moray Firth',]
...
gadm <- readRDS("/home/kpalmer/PHD/PhD/Chapter 4/R Code/GBR_adm1.rds") 
gadm=spTransform(gadm, crs.geo)
ScotMap_highRes <- gadm[gadm$NAME_1 == "Scotland" |gadm$NAME_1 ==  'England', ] #Possibly a ploygone already?
...
dep_map = ggplot() + 
  theme_bw() +
  # Scotland high resolution
  geom_polygon(data=ScotMap_highRes,
               aes(long,lat, group=group), fill = 'darkseagreen') +
  coord_map(xlim = c(-5,-1.25),ylim = c(55.7,58.75))+
  
  # SAC
  geom_polygon(data = MF_df, aes(long,lat), fill = 'yellow')+
  
  # Contour lines
  geom_contour(data=northsea_fort, aes(x=x, y=y, z=z),
               breaks=contour_levels,
               colour="gray18", size=0.1) +
  
  # CPOD deployment locations and text name
  geom_point(data = data.frame(meta2), 
             aes(Lon.1, Lat.1), shape =19, color ='red', size =2)+
  geom_text(data = zz, aes(x = x, y=y, label=group), size=3.5) +
  
  # River Names
  geom_point(data = riverlocsdf, aes(lonDeg,LatDeg), col='blue', size=2) +
  geom_text(data = river_titles_df[c(1,2,4,5,7),], aes(x = x, y=y, label=text),
            size=3.5) +
  ...
  scalebar( x.min = min(northsea_fort$x), 
            x.max = max(northsea_fort$x),
            y.min = min(northsea_fort$y), 
            y.max = max(northsea_fort$y),
            dist = 25, 
            dd2km= TRUE,
            model ='WGS84',
            st.size = 3, height = .01, 
            anchor =c(x =-3.4, y = 55.9))+
  labs(x='', y='', title = 'East Coast Marine Mammal Acoustic Study',
       subtitle= 'C-POD Deployment Locations') 

 

fname =('deployment_locations.tiff') 
ggsave(dep_map, filename = fname, dpi = 300, device = 'tiff')
...
basemap = ggplot() + 
  ...
  xlab('')+
  ylab('')
north2(n, x=.75, y=.85, symbol=16)

dat2 = subset(data.frame(aa), 
              DistToShore>= DistToShoreRange[1] & DistToShore <= DistToShoreRange[2] &
              Slope>= SlopeRange[1] & Slope <= SlopeRange[2] &
              DistToSalmonRun>= DistToFeatureRange[1] & DistToSalmonRun <= DistToFeatureRange[2] &
              Depth.depth>= DepthRange[1] & Depth.depth <= DepthRange[2])


# Basemap for the prediction plots 
Predictions_Basemap = 
  basemap + 
  # Add river locations and text
  geom_point(data = riverlocsdf, aes(lonDeg,LatDeg), col='blue', size=3) +
  geom_text(data = river_titles_df, aes(x = x, y=y, label=text)) 
...
for(ii in 1:3){
  ...
  Preddat=data.frame(SlopeMap=dat2$Slope, 
                     Depth_m=dat2$Depth.depth,
                     DistToSalmonRun=dat2$DistToSalmonRun,
                     BNDTotOffset=rep(0, nrow(dat2)),
                     DistToShore=dat2$DistToShore$distance,
                     Season= unique(OccTable_daily$Season)[ii],
                     JulienDay=jdayval[ii],
                     RiverName=dat2$RiverName,
                     Year=2013)
  ...
  dat3=cbind(dat2, preds)
  

  
  # Prediciton values
  mapdata <- data.frame(dat3)
  mapdata$breaks_fit = cut(mapdata$fit, breaks = seq(0,1, length.out = 20))
  mapdata$breaks_fit_lci = cut(mapdata$LCI, breaks = seq(0,1, length.out = 20))
  title_val = paste(unique(OccTable_daily$Season)[ii],'Acoustic Occupancy Model Predictions')
  Subtitle_val = 'Model Average'
  
  
  # Model average plot
  p<-Predictions_Basemap +
    geom_tile(data = mapdata, 
              aes(x = Lon, y = Lat, fill = log10(fit)), size =.7) + 
    scale_fill_gradientn( 
      colors = viridis_pal()(9), limits=c(log10(.0001), log10(1)),
      ...
      title=expression(paste("log"[10]^{}," (Occupancy)"), paste =''),
      title.hjust = 0.5,
      label.hjust = 0.5)
    )+
    labs(title = title_val, subtitle= Subtitle_val)

   
   fname =paste('modelpredictions_', unique(OccTable_daily$Season)[ii], 'fit.tiff', sep='')
   ggsave(p,filename = fname, dpi = 300, device = 'tiff')
  
  
   # Lower 95% CI map
   Subtitle_val = 'Lower 95% Model Confidence Interval'
   
   plci<-Predictions_Basemap +
     geom_tile(data = mapdata, 
               aes(x = Lon, y = Lat, fill = log10(LCI)), size =.7) + 
     scale_fill_gradientn( 
       colors = viridis_pal()(9), limits=c(log10(.0001), log10(1)),
       guide = guide_colorbar(
         #direction = "horizontal",
  ...
  fname =paste('modelpredictions_lci_', unique(OccTable_daily$Season)[ii], 'fit.tiff', sep='') 
  ggsave(plci,filename = fname, dpi = 300, device = 'tiff')
  

  # Upper Confidence interval Plot
  Subtitle_val = 'Upper 95% Model Confidence Interval'
  
  puci<-Predictions_Basemap +
    geom_tile(data = mapdata, 
              aes(x = Lon, y = Lat, fill = log10(UCI)), size =.7) + 
    scale_fill_gradientn( 
      colors = viridis_pal()(9), limits=c(log10(.0001), log10(1)),
      guide = guide_colorbar(
        #direction = "horizontal",
  ...
  fname =paste('modelpredictions_uci_', unique(OccTable_daily$Season)[ii], 'fit.tiff', sep='') 
  ggsave(puci,filename = fname, dpi = 300, device = 'tiff')

    
  }

tiff(filename = 'Figure 5 GAMPLOT.tiff',  width = 8, height = 10, 
     units = 'in', res = 300)

par(mfrow=c(2, 2)) 
# xlab=""
plot.gam(FinalModel$gam, trans= inv.logit, select=1, all.terms=T, shade=T, 
         xlab="Distance to Point of Aggregation (m)", ylab="P(Broadband)") 
# xlabs=""
plot.gam(FinalModel$gam, trans= inv.logit, select=2, all.terms=T, shade=T,
         xlab="Depth (m)", ylab="")

plot.gam(FinalModel$gam, trans= inv.logit, select=3, all.terms=T, shade=T, 
         xlab="Distance to Shore (m)", ylab="P(Broadband)")
...
rescaler = function(x, to = c(0, 1), from = NULL) {
      ifelse(x<1.2, scales::rescale(x, to = to, from = c(min(x, na.rm = TRUE), 1.2)),
         1)}
  
  

  # Distance Scale Bar and North Arrow
  scalebar(x.min = min(northsea_fort$x), x.max = max(northsea_fort$x),
           y.min = min(northsea_fort$y), y.max = max(northsea_fort$y),
           dist=25, dd2km = TRUE, model = 'WGS84', 
           height =.01, 
           anchor =c(x =-3.6, y = 55.6),
           st.size = 4) +
  north2(x.min = min(northsea_fort$x), x.max = max(northsea_fort$x),
        y.min = min(northsea_fort$y), y.max = max(northsea_fort$y),
        anchor =c(x =-1.7, y = 58.6), symbol = 16) +
...
for(ii in 1:3){
  ...
  dat2 = subset(dat, DistToShore>= DistToShoreRange[1]-710 & DistToShore <= DistToShoreRange[2]+710 &
                  Slope>= SlopeRange[1] & Slope <= SlopeRange[2] &
                  DistToSalmonRun>= DistToFeatureRange[1]-710 & DistToSalmonRun <= DistToFeatureRange[2]+710 &
                  Depth.depth>= DepthRange[1] & Depth.depth <= DepthRange[2])
  ...
  dat2=cbind(dat2, preds)

  rbPal <- colorRampPalette(c('red','blue'))
  #dat1$Col <- rbPal(10)[as.numeric(cut(dat1$fit,breaks = 20))]

  dat2$Col <- viridis(20)[as.numeric(cut(dat2$fit,
                                             breaks = as.numeric(quantile(unlist(preds), seq(0, 1, length.out = 20)))))]

  dat2$ColLCI <- viridis(20)[as.numeric(cut(dat2$LCI,
                                             breaks = as.numeric(quantile(unlist(preds), seq(0, 1, length.out = 20)))))]
  
  dat2$ColUCI <- viridis(20)[as.numeric(cut(dat2$UCI,
                                                breaks = as.numeric(quantile(unlist(preds), seq(0, 1, length.out = 20)))))]
  
  dat2$Season=as.character(Preddat$Season)
  ...
  par(mar=c(3,4,3,6),oma=c(3,1,0,0),mfrow=c(1,1))
  
  layout(t(1:1), widths=c(5,1))
  tiff(file = paste(Preddat$Season[1], 'tff', sep='.'),
       width = 4.5, height = 4, units = "in", res = 800, pointsize = 4.2) 
  
  plot(NorthSea, image = TRUE, lwd =.01, n=0,
       bpal = list(c(0.001, max(NorthSea), 'gray'),
                   c(min(NorthSea),0.001,blues)),
        land =TRUE,
       main = 'Habitat Model Predictions')
  
  scaleBathy(NorthSea, deg=.79, x="bottomleft", y=NULL, inset=10, angle=90)
  
  
  points(x = dat2$Depth.lon, y=dat2$Depth.lat,
       pch = 15, cex=.45,
       col = dat2$Col, main=Preddat$Season[1])
  points(river_locs, pch=19, col='blue')
  plot(NorthSea, n=10, lwd =.5, add = TRUE)
  points(meta2,
         pch = 20, cex=1,
         col = 'red')
  text(river_locs_text, river_locs_text$Rivername)
#  image.plot(1,legend.only=TRUE, zlim= c(0, 1), 
#              col = viridis(20), 
#              legend.lab = "Acoustic Occupancy", horizontal = FALSE) 
  color.legend(-1,55.5,-1.05,58,legend='asdf', viridis(20), gradient='y')
  dev.off()
  ...
  points(x = dat2$Depth.lon, y=dat2$Depth.lat,
         pch = 15, cex=1,
         col = dat2$ColLCI, main=Preddat$Season[1])
