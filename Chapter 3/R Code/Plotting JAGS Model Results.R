...
# Integration time
te=c(1500, 5000,8000)*10e-6

# integration time in miliseconds
te_ms= te*1000

# Add Proper Lables
SM_samps$ModelName=factor(SM_samps$Model, labels=c('null', paste(as.character(te_ms), 'ms', sep='' )))

SM_samps$LocName=factor(SM_samps$UnitLoc, labels=c('St Abbs', 'St Andrews', 'Arbroath', 
                                                   'Stonehaven', 'Cruden Bay', 'Fraserburgh', 
                                                   'Spey Bay', 'Cromarty', 'Helmsdale', 'Latheron'))

SM_samps$LocName_order = factor(SM_samps$LocName, levels=(levels(SM_samps$LocName)))

# mm=SM_samps[! SM_samps$UnitLoc %in% c('Hel_15', 'SpB_10', 'Abr_10', 'StA_10'), ]

# True Occupancy 
 ggplot(data=SM_samps, aes(Model, P.occ)) +
   facet_wrap(~LocName_order, nrow = 2) + 
...
# Relative Occupancy  
  ...
  ggplot(data=SM_samps, aes(Model, Scaled)) +
   ...
   ggplot(data=SM_samps, aes(LocName_order, P.occ)) +
     facet_wrap(~ModelName, nrow = 1, scale='free_x') +
     scale_y_continuous(breaks=pretty_breaks()) +
     coord_flip() +
     guides(fill=FALSE) +
     geom_violin(aes(fill=Model), scale = "width" , draw_quantiles = c(0.25, 0.5, 0.75)) +
     scale_fill_manual(values=c('steelblue3',rep("white", length(unique(SM_samps$Model))-1))) +
     theme_bw() +
     theme(plot.title = element_text(hjust = 0.5),
           panel.spacing = unit(.9, "lines")) +
     #ggtitle('Run 1- C-POD Detections and Noise Levels') +
     ggtitle('Posterior Model Distributions ') +
     labs(x='', y='Estimated Occupancy ')  
